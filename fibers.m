function visualize_fibers(Fibers, Omega)
...
%   Omega:  The mesh structure (for context/plotting)
    ...
    % Use triplot for the triangular mesh (gray lines)
    triplot(Omega.t(:,1:3), Omega.x(:,1), Omega.x(:,2), ...
            'Color', [0.8 0.8 0.8], 'LineWidth', 0.5);
    hold on;
    
    % 2. Extract Fiber Data
    % Check if data is at Nodes or Elements
    ...
    step = 1;
    if size(coords, 1) > 1000, step = 5; end
    
    % 5. Quiver Plot
    quiver(coords(1:step:end, 1), coords(1:step:end, 2), ...
           vecs_norm(1:step:end, 1), vecs_norm(1:step:end, 2), ...
           0.5, 'r', 'LineWidth', 1.5);
